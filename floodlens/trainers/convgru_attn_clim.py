"""convgru_attn_clim trainer on the 50 km GOES/GLM feature grid (config.CACHE_DIR).

Per-frame 2D CNN + ConvGRU + temporal attention, PLUS a per-cell climatology prior.
Identical to `convgru_attn` except a `ClimHead` adds a per-cell, train-only
climatology logit prior — `logit(historical flood-frequency)` — gated by a single
learnable scalar, so the net learns the *deviation* from each cell's base rate
instead of the absolute rate. The prior is derived only from the cached labels
(`_y.npy`) of the current fold's TRAIN days, so no cache rebuild is needed and there
is no val/test leakage. Motivation: the pure-signature model fires on plausible
convection everywhere (most false positives fall in genuinely flood-prone cells);
the prior lets it suppress convection over cells that historically rarely flood.

Inputs: seq (B,8,19,59,95), daily summaries (B,8,59,95), lead (B,8); output per-cell
flood logits (B,1,59,95). Features are normalized (train-only: log1p on GLM, then
standardize) inside the model; lead time is added as a per-frame channel. Loss =
0.7 focal + 0.25 soft-CSI + 0.05 spatial-tolerance (pos_weight 50). Train across both
GPUs:

    cd notebooks/model
    NCCL_P2P_DISABLE=1 torchrun --nproc_per_node=2 trainers/convgru_attn_clim.py
...
CLIM_EPS = 1e-4       # floor/ceil on flood-frequency before logit (avoid +/-inf)
...
#           + per-cell climatology prior (ClimHead) gated by a learnable scalar
# ===========================================================================
class FloodNet(nn.Module):
    """Inputs: seq (B,8,19,R,C), summ (B,8,R,C), t (B,8). Output (B,1,R,C) logits.

    Output = signature-head logits + clim_scale * clim_prior, where clim_prior is a
    frozen per-cell buffer set to logit(train flood-frequency) and clim_scale is one
    learnable scalar (init 1.0). The net thus learns each cell's *deviation* from its
    historical base rate rather than the absolute rate.
    """
    ...
    def __init__(self):
        ...
        self.register_buffer("clim_prior", torch.zeros(1, 1, GRID_R, GRID_C))
        self.clim_scale = nn.Parameter(torch.ones(1))         # learnable scalar gate
    ...
    def set_clim(self, clim_logit):
        """clim_logit: (R,C) per-cell logit(train flood-frequency)."""
        self.clim_prior.copy_(torch.as_tensor(clim_logit, dtype=torch.float32)
                              .view(1, 1, GRID_R, GRID_C))
    ...
    def forward(self, seq, summ, t):
        ...
        logits = self.head(fuse)                               # (B,1,R,C)
        return logits + self.clim_scale * self.clim_prior      # add per-cell prior


# ===========================================================================
# Loss = 0.7 focal + 0.25 soft-CSI + 0.05 spatial-tolerance  (over land cells)
# ===========================================================================
...
# ===========================================================================
# Data + train-only feature stats + train-only climatology prior
# ===========================================================================
...
def load_or_compute_clim(days, land):
    """Train-only per-cell logit(flood-frequency) prior; cached to disk per fold.

    Reads only the cached labels (`_y.npy`) of the fold's TRAIN days — no cache
    rebuild, no val/test leakage. Off-land cells are 0 frequency -> floored logit.
    """
    fp = CACHE_DIR / f"clim_prior{fold_suffix()}.npy"
    if fp.exists():
        return np.load(fp)
    acc = np.zeros((GRID_R, GRID_C), np.float64)
    for d in days:
        acc += np.load(CACHE_DIR / f"{d}_y.npy").astype(np.float64)
    freq = acc / max(len(days), 1)
    freq = np.clip(freq, CLIM_EPS, 1.0 - CLIM_EPS)
    clim_logit = np.log(freq / (1.0 - freq)).astype(np.float32)   # logit(freq)
    clim_logit[~land] = np.log(CLIM_EPS / (1.0 - CLIM_EPS))       # off-land -> floor
    np.save(fp, clim_logit)
    return clim_logit
...
def main():
    ...
    if is_main:
        ...
        load_or_compute_clim(tr_days, land)                   # ensure clim prior exists
    if ddp:
        dist.barrier()
    stats = load_or_compute_stats(tr_days, land)
    clim = load_or_compute_clim(tr_days, land)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if is_main:
            ...
            print(f"[{NAME}] ep {epoch:2d}/{EPOCHS} lr {cur_lr:.1e} "
                  ...
                  f" clim_scale {net.clim_scale.item():.2f}"
    ...
    if is_main:
        ...
        print(f"[{NAME}] FINAL  best-val @ epoch {best_epoch}  "
              f"thr {vm['thr']:.3f} (best-val F1)  clim_scale {net.clim_scale.item():.3f}")
