...
lead time is added as a per-frame channel. Loss = Tversky only
(alpha=0.3 FP, beta=0.7 FN -> recall-favoring for the rare-positive target).
Train across both GPUs:

    cd notebooks/model
    NCCL_P2P_DISABLE=1 torchrun --nproc_per_node=2 trainers/convgru_attn_tversky.py
...
LOSS_CSI_W = 0.25
LOSS_TOL_W = 0.05
# Tversky loss only (recall-favoring for the rare-positive target): alpha weights
# false positives, beta weights false negatives; beta > alpha penalizes misses more.
TVERSKY_ALPHA = 0.3
TVERSKY_BETA = 0.7
DROPOUT = 0.2
WEIGHT_DECAY = 1e-2
...
def _tversky_loss(probs, y, alpha=TVERSKY_ALPHA, beta=TVERSKY_BETA, eps=1.0):
    """1 - Tversky index. alpha*FP + beta*FN; beta > alpha favors recall."""
    probs = probs.float()
    y = y.float()
    tp = (probs * y).sum()
    fp = (probs * (1 - y)).sum()
    fn = ((1 - probs) * y).sum()
    return 1.0 - tp / (tp + alpha * fp + beta * fn + eps)


def combined_loss(logits, y, land_t, pos_weight):
    """Tversky loss only (over land cells)."""
    probs = torch.sigmoid(logits.float())
    yf = y.float()[:, land_t]
    return _tversky_loss(probs[:, land_t], yf)
