"""convgru trainer on the 50 km GOES/GLM feature grid (config.CACHE_DIR).

Per-frame 2D CNN + ConvGRU.
...
spatial-tolerance. Train across both GPUs:
    ...
    cd notebooks/model
    NCCL_P2P_DISABLE=1 torchrun --nproc_per_node=2 trainers/convgru.py
...
class Encoder(nn.Module):
    """Per-frame 2D CNN, then a ConvGRU recurring over the 8 frames."""
    ...
    def forward(self, seq):                                   # (B,T,SEQ_IN,R,W)
        ...
        for tt in range(Tn):
            h = self.cell(x[:, tt], h)
        return self.drop(h)                                  # (B,ENC_OUT,R,W)
