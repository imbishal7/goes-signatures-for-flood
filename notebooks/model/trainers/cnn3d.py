"""cnn3d trainer on the unified GOES cache (config.CACHE_DIR).

Plain 3D-CNN image encoder (full 3x3x3 convolutions).
...
class Encoder(nn.Module):
    """Plain 3D-CNN over (T,H,W): /8 in space, time -> 1."""

    def __init__(self):
        super().__init__()
        self.net = nn.Sequential(
            nn.BatchNorm3d(N_IMG),
            nn.Conv3d(N_IMG, 32, (3, 5, 5), padding=(1, 2, 2)),
            nn.BatchNorm3d(32), nn.ReLU(inplace=True),
            nn.MaxPool3d((1, 2, 2)),                          # 8,375,625
            nn.Conv3d(32, 64, 3, padding=1),
            nn.BatchNorm3d(64), nn.ReLU(inplace=True),
            nn.MaxPool3d((2, 2, 2)),                          # 4,187,312
            nn.Conv3d(64, 128, 3, padding=1),
            nn.BatchNorm3d(128), nn.ReLU(inplace=True),
            nn.MaxPool3d((2, 2, 2)),                          # 2,93,156
            nn.Dropout3d(DROPOUT),
            nn.Conv3d(128, 256, 3, padding=1),
            nn.BatchNorm3d(256), nn.ReLU(inplace=True),
            nn.MaxPool3d((2, 1, 1)),                          # 1,93,156
        )
