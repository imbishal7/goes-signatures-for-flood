"""Model #3 — 3D CNN (full 3D convolutions) (GOES -> next-day warning grid).
...
This is the full-3D sibling of r2plus1d_train.py: instead of factorizing each kernel
into space-then-time, it uses true (3,3,3) convolutions. The encoder
...
(Single-GPU also works: `python trainers/cnn3d.py`.)
...
EPOCHS = 50
BATCH_SIZE = 4                 # per GPU (~48 GB peak measured); fits under 96 GB
WORKERS = 4
...
class GOES3DCNN(nn.Module):
    """3D-CNN encoder over (time, H, W) -> collapse time -> CellPool regrid -> head.

    Input  : (B, T=8, C=n_ch, 1500, 2500)
    Output : (B, 1, GRID_R, GRID_C) raw logits (apply sigmoid for probability).

    Read top to bottom: a plain stack of full (3,3,3) convolutions (BatchNorm3d + ReLU).
    Maxpools shrink space to /8 and collapse the 8 frames to 1; CellPool then regrids
    onto the 50 km grid and a 2D head emits the per-cell logit.
    """
    ...
    def __init__(self, pix2cell_sub, grid_r, grid_c, n_ch=N_CH):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Conv3d(n_ch, 32, (3, 5, 5), padding=(1, 2, 2)),
            ...
            nn.MaxPool3d((2, 1, 1)),                            # B,256,1,187,312 (t->1)
        )
    ...
    def forward(self, x):                                          # B, T, C, H, W
        ...
        x = self.encoder(x).squeeze(2)                         # -> B, 256, ENC_H, ENC_W
        x = self.pool(x)                                           # -> B, 256, R, C
...
def main():
    ...
    net = GOES3DCNN(pix_sub, gr, gc).to(dev)
    if ddp:
    ...
    if is_main:
        ...
        ckpt = OUT_DIR / "cnn3d.pt"
        torch.save(net.state_dict(), ckpt)
        np.savez(OUT_DIR / "cnn3d_results.npz", hist=np.array(hist, np.float32))
