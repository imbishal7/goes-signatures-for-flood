"""logreg baseline on the 50 km GOES/GLM feature grid (config.CACHE_DIR).

Per-cell tabular baseline (pure GOES/GLM signatures): linear logistic regression.
168 feats/(cell,day) = seq 8*19 + daily-sum 8 + lead 8. Imbalance via class_weight;
...
def fit_model(Xtr, Ytr, Xva, Yva, spw):
    ...
    model = make_pipeline(
        StandardScaler(),
        LogisticRegression(max_iter=300, class_weight={0: 1.0, 1: spw}, n_jobs=-1),
    )
    model.fit(Xtr, Ytr)
    return model
