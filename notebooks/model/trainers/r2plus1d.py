"""Model #2 — factorized 3D CNN, R(2+1)D (GOES -> next-day warning grid).
...
R(2+1)D factorizes each 3D convolution into a **spatial (1,3,3)** conv followed by a
**temporal (3,1,1)** conv. That decomposition has far fewer parameters than a full
(3,3,3) kernel, adds a ReLU between the spatial and temporal steps (more nonlinearity),
and tends to optimize more easily — while still mixing space and time. The encoder
shrinks space to /8 and collapses the 8 frames to 1, CellPool regrids those pixels onto
the 50 km grid, and a 2D head emits the per-cell logit.
...
(Single-GPU also works: `python trainers/r2plus1d.py`.)
...
# Encoder spatial downsample: 1500x2500 -> ENC_H x ENC_W; CellPool then regrids those
# pixels onto the 50 km grid. /8 leaves ~10 pixels per cell (finer than the grid).
ENC_DOWN = 8
ENC_H, ENC_W = 1500 // ENC_DOWN, 2500 // ENC_DOWN          # 187 x 312

EPOCHS = 50
BATCH_SIZE = 3                 # per GPU (~53 GB peak measured); fits under 96 GB
WORKERS = 4                    # dataloader processes

# LR: short linear warmup then cosine decay (warm start avoids early divergence; the
# decay lets it settle as it converges).
LR = 3e-4                      # peak LR (reached at the end of warmup)
MIN_LR = 1e-5                 # floor the cosine decays to
WARMUP_EPOCHS = 1
POS_WEIGHT_CAP = 100.0        # cap on the BCE positive weight (= (1-rate)/rate)
SEED = 0
...
# Model
# ===========================================================================
class GOESR2plus1d(nn.Module):
    """R(2+1)D encoder over (time, H, W) -> collapse time -> CellPool regrid -> head.

    Input  : (B, T=8, C=n_ch, 1500, 2500)
    Output : (B, 1, GRID_R, GRID_C) raw logits (apply sigmoid for probability).

    Read top to bottom: four factorized 3D "blocks", each a spatial (1,3,3) conv then a
    temporal (3,1,1) conv (BatchNorm3d + ReLU each). Maxpools shrink space to /8
    and collapse the 8 frames to 1; CellPool then regrids onto the 50 km grid.
    """

    def __init__(self, pix2cell_sub, grid_r, grid_c, n_ch=N_CH):
        super().__init__()
        self.encoder = nn.Sequential(
            nn.BatchNorm3d(n_ch),                                # normalise band scales
            # block 1 (spatial conv, then temporal conv)
            nn.Conv3d(n_ch, 32, (1, 3, 3), padding=(0, 1, 1), bias=False),
            nn.BatchNorm3d(32), nn.ReLU(inplace=True),
            nn.Conv3d(32, 32, (3, 1, 1), padding=(1, 0, 0), bias=False),
            nn.BatchNorm3d(32), nn.ReLU(inplace=True),
            nn.MaxPool3d((1, 2, 2)),                               # B,32, 8,750,1250
            # block 2
            nn.Conv3d(32, 64, (1, 3, 3), padding=(0, 1, 1), bias=False),
            nn.BatchNorm3d(64), nn.ReLU(inplace=True),
            nn.Conv3d(64, 64, (3, 1, 1), padding=(1, 0, 0), bias=False),
            nn.BatchNorm3d(64), nn.ReLU(inplace=True),
            nn.MaxPool3d((2, 2, 2)),                               # B,64, 4,375,625
            # block 3
            nn.Conv3d(64, 128, (1, 3, 3), padding=(0, 1, 1), bias=False),
            nn.BatchNorm3d(128), nn.ReLU(inplace=True),
            nn.Conv3d(128, 128, (3, 1, 1), padding=(1, 0, 0), bias=False),
            nn.BatchNorm3d(128), nn.ReLU(inplace=True),
            nn.MaxPool3d((2, 2, 2)),                              # B,128,2,187,312 (/8)
            # block 4
            nn.Conv3d(128, 128, (1, 3, 3), padding=(0, 1, 1), bias=False),
            nn.BatchNorm3d(128), nn.ReLU(inplace=True),
            nn.Conv3d(128, 128, (3, 1, 1), padding=(1, 0, 0), bias=False),
            nn.BatchNorm3d(128), nn.ReLU(inplace=True),
            nn.MaxPool3d((2, 1, 1)),                            # B,128,1,187,312 (t->1)
        )
    ...
    def forward(self, x):                                          # B, T, C, H, W
        ...
        x = self.encoder(x).squeeze(2)                         # -> B, 128, ENC_H, ENC_W
        x = self.pool(x)                                           # -> B, 128, R, C
        return self.head(x)                                     # -> B, 1, R, C (logits)
...
def main():
    ...
    net = GOESR2plus1d(pix_sub, gr, gc).to(dev)
    if ddp:
    ...
    if is_main:
        ...
        ckpt = OUT_DIR / "r2plus1d.pt"
        torch.save(net.state_dict(), ckpt)
        np.savez(OUT_DIR / "r2plus1d_results.npz", hist=np.array(hist, np.float32))
        print(f"saved {ckpt} + r2plus1d_results.npz", flush=True)
    if ddp:
