...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Pull CONUS NWS Flash Flood + Flood warnings with polygons."
    )
    ...
    p.add_argument("--out", default=str(DEFAULT_OUT_DIR), metavar="DIR")
    p.add_argument("--workers", type=int, default=DEFAULT_WORKERS, metavar="N")
    p.add_argument(
        "--format", default="both", choices=["parquet", "csv", "both"],
    )
    return p.parse_args()
